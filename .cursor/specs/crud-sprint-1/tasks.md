...
- [x] 4. CounterpartyFormComponent (dumb)
  ...
  - [x] 4.4 Объявить интерфейс `CounterpartyFormModel` и инициализацию формы из `counterparty()` при наличии (edit mode)
  - [x] 4.5 Добавить все поля формы
  - [x] 4.6 Реализовать метод `validate()`: обязательные поля legalForm, name, inn, status, role (хотя бы одна роль)
  - [x] 4.7 Реализовать метод `submit()`: вызов `createCounterparty()` или `updateCounterparty()`
  - [x] 4.8 Реализовать метод `lookupByInn()` и обработку 404/общих ошибок
  ...
  - [x] 4.11 Импортировать UI-компоненты из `shared/ui`
...
- [x] 5. CounterpartiesComponent (smart)
  ...
  - [x] 5.3 Реализовать `ngOnInit()`
  - [x] 5.4 Реализовать реактивный поиск и фильтрацию через debounce 300ms
  - [x] 5.5 Реализовать методы create/edit/delete
  - [x] 5.6 В `onSaved()`: добавить или заменить сущность, закрыть форму
  - [x] 5.7 В `onDeleteConfirmed()`: API-вызов, удаление из списка, `NotificationService.error()` при ошибке
  - [x] 5.8 Использовать `takeUntilDestroyed()` для подписок
  - [x] 5.9 Реализовать шаблон страницы с таблицей/формой/подтверждением
  - [x] 5.10 Отображать `AlertComponent`, loading-индикатор, empty-state
  - [x] 5.11 Импортировать требуемые компоненты
  - [x] 5.12 Инжектировать `NotificationService`

- [x] 6. Ручная проверка
  - [x] 6.1 Проверить навигацию по сайдбару (/, /products, /counterparties, /settings)
  - [x] 6.2 Проверить мобильный collapse сайдбара < 768px
  - [x] 6.3 Проверить CRUD контрагентов
  - [x] 6.4 Проверить DaData lookup (успех, 404, ошибка)
  - [x] 6.5 Проверить поиск и фильтрацию с debounce
  - [x] 6.6 Проверить `@media print` скрытие сайдбара
