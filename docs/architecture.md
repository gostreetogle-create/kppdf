...
|-------------------------|-------------------------------------------------------|
| `signal()` / `computed()`| Основное состояние компонентов и вычисляемые итоги    |
| `effect()`              | Автосохранение, синхронизация фильтров с роутом       |
| `toObservable()`        | Для комбинирования сигналов в RxJS-потоки (например, `combineLatest` в `ngOnInit`). **Важно:** при вызове вне конструктора (например, в `ngOnInit`) необходимо передавать `Injector`. |
| `takeUntilDestroyed()`  | Отписка от RxJS-потоков в компонентах и сервисах      |
