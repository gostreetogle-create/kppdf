...
- При создании КП обязательна компания-инициатор (`companyId`), из которой формируется `Kp.companySnapshot` (фоны `kp-page1/kp-page2/passport` и `footerText`); дальнейшие правки карточки компании не изменяют уже созданное КП.
