...
- `title` доминирует, но для плотного каталога допускается compact-типографика (`~13px`, `700`), `sku` остаётся вторичным (`11px`, muted);
...
- карточка может содержать compact photo-slot товара (главное фото или первое доступное) без изменения внешних размеров карточки;
...
- В `Параметры КП` видимость колонки `Фото` переключается compact icon-only кнопкой (глаз), без текста внутри кнопки; состояние читается по цвету/иконке, доступность — через `title` и `aria-label`.
