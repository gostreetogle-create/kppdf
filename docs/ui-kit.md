...
- Для исключения двусмысленности по НДС таблица КП использует явные подписи `Цена (вкл. НДС)` и `Сумма (вкл. НДС)`, а НДС показывается отдельной колонкой `НДС (в т.ч.)`.
 ...
 - В блоке `Получатель` (KP Builder) из snapshot-формы убраны readonly-поля `Название`, `ИНН`, `КПП`; оставлены выбор из справочника и служебная подсказка, чтобы уменьшить визуальный шум.
...
- Таблица `Counterparties` стабилизирована локальным классом `counterparty-table`: фиксированная схема колонок по class-based widths, `col-name` забирает остаток ширины, `col-actions` всегда держит кнопки в одну строку, перенос role-бейджей не сдвигает соседние колонки.
- Для `Counterparties` action-ячейки добавлена защита от клиппинга: `td.col-actions { overflow: visible; }` + `ui-table-actions { min-width: max-content; }`, чтобы `Изменить/Удалить` всегда рендерились полностью.
- В таблице `Counterparties` колонка `Название` отображает только короткое имя (`shortName`, fallback на `name`) без второй строки с полным наименованием для более плотного списка.
- В `Counterparties` применён пропорциональный `column sizing` (в процентах) с расширенным `col-actions` (`min-width`), чтобы не оставлять избыточную пустую зону в центре и не обрезать действия справа.
- Для стабильного правого края `Counterparties` action-колонка закреплена на `16%` (`min-width: 190px`), а контейнер кнопок рендерится внутри ячейки (`display:flex; width:100%`) без overflow за пределы таблицы.
...
- В форме контрагента (для роли `Наша компания`) добавлен чекбокс `Компания по умолчанию для создания КП`; Home использует этот признак для предвыбора компании-инициатора.
