...
- stepper реализуется как единый compact-control: ширина ~`76px`, общий border + внутренние разделители между `- / qty / +`, `border-radius:4px`;
...
- stepper и узкие input-контролы должны опираться на `--input-bg` + `--ui-border` (читаемо, но без агрессивной рамки);
