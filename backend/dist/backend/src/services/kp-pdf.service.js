...
function renderKpHtml(kp) {
    ...
    const subtotal = items.reduce((sum, item) => sum + (Number(item?.qty) || 0) * (0, price_utils_1.calculateItemUnitPrice)(item), 0);
    ...
    const rows = items.map((item, index) => {
        ...
        const price = (0, price_utils_1.calculateItemUnitPrice)(item);
    ...
    const conditionsHtml = conditions.length
      ...
      <section class="conditions">
        <h3>Условия</h3>
        ${conditions.map((condition) => `<p>${escapeHtml(condition)}</p>`).join('')}
      </section>
    `
        : '';
    return `<!doctype html>
...
class KpPdfService {
    async generatePdf(kp) {
        const kpId = String(kp?._id || '');
        const updatedAt = kp?.updatedAt || kp?.metadata?.createdAt || new Date();
        const cachePath = kpId ? pdf_cache_service_1.pdfCacheService.getCachePath(kpId, updatedAt, 'kp') : null;
        if (cachePath) {
    ...
    async performGenerate(kp, cachePath) {
...
exports.KpPdfService = KpPdfService;
exports.kpPdfService = new KpPdfService();
