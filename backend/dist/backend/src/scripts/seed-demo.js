...
// ─── Helpers ──────────────────────────────────────────────────────────────────
function randomItems(products, count) {
...
function makeKp(title, status, counterparty, products, num) {
...
async function run() {
    ...
    const existingProducts = await product_model_1.Product.countDocuments();
    if (existingProducts > 0) {
        console.log(`ℹ️  В БД уже есть ${existingProducts} товаров. Пропускаю seed товаров.`);
    }
    else {
        const products = await product_model_1.Product.insertMany(PRODUCTS.map(p => ({ ...p, images: [], isActive: true })));
        console.log(`✅ Создано ${products.length} товаров`);
    }
    const existingCp = await counterparty_model_1.Counterparty.countDocuments();
    if (existingCp > 0) {
        console.log(`ℹ️  В БД уже есть ${existingCp} контрагентов. Пропускаю.`);
    }
    else {
        const cps = await counterparty_model_1.Counterparty.insertMany(COUNTERPARTIES.map(c => ({ ...c, contacts: [], tags: [], sameAddress: false })));
    ...
    // КП
    const existingKp = await kp_model_1.Kp.countDocuments();
    if (existingKp > 0) {
        console.log(`ℹ️  В БД уже есть ${existingKp} КП. Пропускаю.`);
    }
    else {
        const allProducts = await product_model_1.Product.find();
        const allCp = await counterparty_model_1.Counterparty.find();
        const kps = [
        ...
        await kp_model_1.Kp.insertMany(kps);
