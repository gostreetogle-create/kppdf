...
exports.productPassportPdfService = exports.ProductPassportPdfService = void 0;
    ...
    <section class="page">
      ...
      ${groups.map((group) => `
        <div class="spec-section">
          ...
          <table>
            <tbody>
              ${(Array.isArray(group?.params) ? group.params : []).map((param) => `
                <tr>
...
class ProductPassportPdfService {
    async getConstants() {
        const defaults = {
        ...
        const defaultByKey = new Map(settings_model_1.DEFAULT_SETTINGS.map((item) => [item.key, item.value]));
        const settings = await settings_model_1.Setting.find({ key: { $in: ['passport_warranty_text', 'passport_storage_text'] } }).lean();
    ...
    async generateByProductId(productId) {
        const product = await product_model_1.Product.findById(productId).lean();
        if (!product)
            throw new Error('Товар не найден');
        const spec = await product_spec_model_1.ProductSpec.findOne({ productId }).lean();
        if (!spec)
            throw new Error('Технический профиль не найден');
        ...
        const cachePath = pdf_cache_service_1.pdfCacheService.getCachePath(productId, latestUpdate, 'passport');
...
exports.ProductPassportPdfService = ProductPassportPdfService;
exports.productPassportPdfService = new ProductPassportPdfService();
