...
router.post('/', async (req, res) => {
    try {
        const cp = await counterparty_model_1.Counterparty.create(req.body);
...
router.put('/:id', async (req, res) => {
    try {
        const cp = await counterparty_model_1.Counterparty.findByIdAndUpdate(req.params.id, req.body, { new: true, runValidators: true });
        if (!cp) {
            res.status(404).json({ message: 'Контрагент не найден' });
            return;
        }
