...
export class PdfGeneratorService {
  ...
  async generateKpPdf(input: GenerateKpPdfInput): Promise<Buffer> {
    ...
    const topItemsCount = ((kp as any)?.items?.length ?? 0) as number;
    ...
    try {
      ...
      const preDiagnostics = await page.evaluate(() => {
        const marker = (globalThis as any).__KP_PDF_READY__;
      ...
      console.log(
        `[PDF pre] KP ${input.kpId}: expectedItems=${expectedItems}, markerItemsCount=${preDiagnostics.markerItemsCount}, htmlItemsCount=${preDiagnostics.htmlItemsCount}, domRows=${preDiagnostics.domRows}`
      );
      ...
      await page.waitForFunction(
        (itemsCount) => {
          const marker = (globalThis as any).__KP_PDF_READY__;
          if (!marker || typeof marker.itemsCount !== 'number') return false;
          if (itemsCount > 0) return marker.itemsCount === itemsCount;
      ...
      const diagnostics = await page.evaluate(() => {
        const marker = (globalThis as any).__KP_PDF_READY__;
        const domRows = document.querySelectorAll('table.kp-table tbody tr.kp-row').length;
        const firstRow = document.querySelector('table.kp-table tbody tr.kp-row') as HTMLTableRowElement | null;
        const firstRowCells = firstRow
          ? Array.from(firstRow.querySelectorAll('td')).map((td) => (td.textContent ?? '').replace(/\s+/g, ' ').trim())
          : [];
        const nameCell = firstRow?.querySelector('.kp-cell-name') as HTMLElement | null;
